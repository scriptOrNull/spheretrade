          ...
          <div className="lg:col-span-2 bg-card border border-border rounded-xl p-4 sm:p-6">
            <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                <span className="text-xs sm:text-sm font-bold text-primary font-mono">{stock.symbol.slice(0, 2)}</span>
              </div>
              <div className="min-w-0">
                <h1 className="text-lg sm:text-xl font-bold text-foreground">{stock.symbol}</h1>
                <p className="text-xs sm:text-sm text-muted-foreground truncate">{stock.company_name}</p>
              </div>
              <div className="ml-auto text-right shrink-0">
                <div className="text-xl sm:text-2xl font-bold font-mono text-foreground">${Number(stock.current_price).toFixed(2)}</div>
                <div className={`flex items-center gap-1 text-xs sm:text-sm ${stock.price_change_pct >= 0 ? 'text-accent' : 'text-destructive'}`}>
                  {stock.price_change_pct >= 0 ? <TrendingUp className="h-3 w-3 sm:h-4 sm:w-4" /> : <TrendingDown className="h-3 w-3 sm:h-4 sm:w-4" />}
            ...
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={chartData}>
                ...
                <YAxis tick={{ fontSize: 10, fill: 'hsl(215 15% 55%)' }} axisLine={false} tickLine={false} domain={['auto', 'auto']} width={50} />
